...
run_program <- function(file){
  ...
  #  if (i < length(masses_p)){
  #    if (abs(masses_p[i+1]-masses_p[i]) <= 1.006355){
  #      filtered_masses <- c(filtered_masses, masses_p[i])
  #      masses_p[i+1] <- NULL
  #      }
  #    else if (abs(masses_p[i+1]-masses_p[i]) <= 0.5065){
  #      n_mass <- (masses_p[i]*2) - 2*1.00794
  #      filtered_masses <- c(filtered_masses, n_mass)
  #      masses_p[i+1] <- NULL
  #      }
  #    else{
  #      filtered_masses <- c(filtered_masses, masses_p[i])
  ...
  #find all entries that matches one or less times
  masses <- c()
  rt <- c()
  times_matched <- c()
  type_match <- c()
  ...
  write.csv(results_df, file= paste0(file_name,"_less_than_one_CENTWAVE.csv"))
  write.csv(all_match_df, file = paste0(file_name,"_allMatches.csv") )
  write.csv(dataframe_p, file = paste0(file_name, "Raw_LC_data.csv"))
  #write.csv(all_match_df, file= paste0(file_name, "_all_matches.csv"))
